...
async function login(req, res, next) {
  ...
  */
      ...
      const jsonTokenExpiration = parseInt(process.env.AUTH_ACCESS_TOKEN_TTL, 10);
      const token = jwt.sign(payloadJwt, process.env.AUTH_JWT_SECRET, { expiresIn: jsonTokenExpiration });
      const response = {
        accessToken: token,
        expiresIn: jsonTokenExpiration,
      };
